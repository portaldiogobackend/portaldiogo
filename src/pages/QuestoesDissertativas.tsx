...
export default function QuestoesDissertativas() {
  ...
  const handleUnassign = async () => {
    ...
    try {
      ...
      console.error('Erro ao cancelar envio da questão:', error);
      showToast(formatErrorMessage(error, 'Erro ao cancelar envio da questão.'), 'error');
    } finally {
      setAssigning(false);
    }
  };
        ...
        {selectedQuestaoIds.length > 0 && (
          <div className="md:hidden fixed bottom-4 left-4 right-4 z-40 rounded-2xl border border-gray-200 bg-white/95 backdrop-blur shadow-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-[#1B2559]">{selectedQuestaoIds.length} questão(ões) selecionada(s)</span>
              <button
                onClick={() => setSelectedQuestaoIds([])}
                className="text-xs font-medium text-red-600 hover:underline"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4 pt-4 border-t border-gray-100">
              ...
              <Button
                ...
                disabled={alunos.length === 0}
              >
                {massAssignMode === 'unassign' ? 'Cancelar envio' : 'Enviar Questão'}
              </Button>
            </div>
          </div>
        ) : null}
